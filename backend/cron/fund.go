...
func SyncFund() {
	...
	for _, fundType := range fundTypes {
		...
		// 更新4433列表
		Update4433(fundlist)
		logrus.Infof("SyncFund %d funds saved to database successfully, fundType:%d", len(fundlist), fundType)
		time.Sleep(1 * time.Minute)
	}

	logrus.Info("SyncFund request end...")
}
...
func Update4433(allFundlist models.FundList) {
	ctx := context.Background()
	fundlist := models.FundList{}
	codesYes := make([]string, 0, len(allFundlist)) // 满足4433
	codesNo := make([]string, 0, len(allFundlist))  // 不满足4433
	for _, fund := range allFundlist {
		if fund.Is4433(ctx) {
			fundlist = append(fundlist, fund)
			codesYes = append(codesYes, fund.Code)
			continue
		}
		codesNo = append(codesNo, fund.Code)
	}
	// 更新 models 变量
	...
	logrus.Infof("Update4433 found %d funds that meet 4433 criteria, %d funds that do not meet 4433 criteria", len(codesYes), len(codesNo))
	// 1) 将不满足 4433 的基金置为 false
	if len(codesNo) > 0 {
		if err := models.DB.Model(&models.FundDB{}).
			Where("code IN ?", codesNo).
			Update("is_4433", false).Error; err != nil {
			logrus.Errorf("Update4433 update is_4433=false error: %v", err)
		}
	}
	// 2) 将满足 4433 的基金置为 true
	if len(codesYes) > 0 {
		if err := models.DB.Model(&models.FundDB{}).
			Where("code IN ?", codesYes).
			Update("is_4433", true).Error; err != nil {
			logrus.Errorf("Update4433 update is_4433=true error: %v", err)
		}
	}

	logrus.Infof("Update4433 request end...")
}
