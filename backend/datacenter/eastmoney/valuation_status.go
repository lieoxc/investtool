...
func (e EastMoney) QueryValuationStatus(ctx context.Context, secuCode string) (map[string]string, error) {
	...
	logrus.WithContext(ctx).WithFields(logrus.Fields{"params": params}).Debug("EastMoney QueryValuationStatus " + apiurl1 + " begin")
	if err != nil {
		return nil, err
	}
	resp := RespValuation{}
	...
	logrus.WithContext(ctx).WithFields(logrus.Fields{"latency(ms)": latency}).Debug("EastMoney QueryValuationStatus " + apiurl1 + " end")
	if resp.Code != 0 {
	...
	logrus.WithContext(ctx).WithFields(logrus.Fields{"params": params}).Debug("EastMoney QueryValuationStatus " + apiurl2 + " begin")
	if err != nil {
		return nil, err
	}
	...
	latency = time.Now().Sub(beginTime).Milliseconds()
	logrus.WithContext(ctx).WithFields(logrus.Fields{"latency(ms)": latency}).Debug("EastMoney QueryValuationStatus " + apiurl2 + " end")
	if resp.Code != 0 {
		return nil, fmt.Errorf("%s %#v", secuCode, resp)
	}
	if len(resp.Result.Data) > 0 {
	...
	logrus.WithContext(ctx).WithFields(logrus.Fields{"params": params}).Debug("EastMoney QueryValuationStatus " + apiurl3 + " begin")
	if err != nil {
		return nil, err
	}
	...
	latency = time.Now().Sub(beginTime).Milliseconds()
	logrus.WithContext(ctx).WithFields(logrus.Fields{"latency(ms)": latency}).Debug("EastMoney QueryValuationStatus " + apiurl3 + " end")
	if resp.Code != 0 {
		return nil, fmt.Errorf("%s %#v", secuCode, resp)
	}
	if len(resp.Result.Data) > 0 {
	...
	logrus.WithContext(ctx).WithFields(logrus.Fields{"params": params}).Debug("EastMoney QueryValuationStatus " + apiurl4 + " begin")
	if err != nil {
		return nil, err
	}
	err = goutils.HTTPGET(ctx, e.HTTPClient, apiurl4, nil, &resp)
	latency = time.Now().Sub(beginTime).Milliseconds()
	logrus.WithContext(ctx).WithFields(logrus.Fields{"latency(ms)": latency}).Debug("EastMoney QueryValuationStatus " + apiurl4 + " end")
	if err != nil {
		return nil, err
	}
	if resp.Code != 0 {
